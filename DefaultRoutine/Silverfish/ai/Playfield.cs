...
namespace HREngine.Bots
{
    ...
    public class Playfield
    {
        ...
        // ===== 新增：地标 =====
        public int locationDurability = 0;
        public bool locationExhausted = false;
        public int locationUsedCount = 0;
        ...
        public void doAction(Action aa)
        {
            ...
            // 创建并执行操作
            Action a = new Action(aa.actionType, ha, o, aa.place, trgt, aa.penalty, aa.druidchoice, aa.titanAbilityNO);
            ...
            switch (a.actionType)
            {
                ...
                //    }
                    ...
                    //break;
                case actionEnum.useLocation:
                    useLocation(a.own, a.target, a.penalty);// 使用地标
                    break;
        ...
        private void HandlePostAttackEffects(Minion attacker, Minion defender, bool dontcount)
        {
            switch (attacker.name)
            {
                ...
                case CardDB.cardNameEN.foereaper4000:
                    ...
                    //if (attacker.enchs.length > 0)
                    //{
                      //  if (attacker.enchs.IndexOf("AV_336e") > 0)
                        //if (attacker.enchs.IndexOf("TTN_079e") > 0)
                        //{
                         //   this.minionGetDamageOrHeal(this.enemyHero, -defender.Hp);
                        //}
                        //if (attacker.enchs.IndexOf("SW_458e") > 0)
                        //{
                        //attacker.immueWhileAttacking = true;
                        //}
                  //  }
                    //break;
            }
        ...
        /// 检查手牌中是否有香菱的创生之星和终结之星配对，如果有则触发
        private void CheckForGDB118Pair()
        {
            //提前检查是否有必要继续，必须同时存在才可能配对
            bool hasType1 = this.owncards.Any(c => c.card.cardIDenum == CardDB.cardIDEnum.GDB_118t);
            bool hasType2 = this.owncards.Any(c => c.card.cardIDenum == CardDB.cardIDEnum.GDB_118t2);

            //如果没有直接返回
            if (!hasType1 || !hasType2 || this.owncards.Count < 2) return;

            //按位置排序手牌
            List<Handmanager.Handcard> sortedHand = this.owncards.OrderBy(card => card.position).ToList();
            bool foundPair = false;

            //遍历手牌检查相邻配对
            for (int i = 0; i < sortedHand.Count - 1; i++)
            {
                Handmanager.Handcard card1 = sortedHand[i];
                Handmanager.Handcard card2 = sortedHand[i + 1];

                //检查是否连续位置
                if (card1.position + 1 != card2.position) continue;

                //合并的配对检查逻辑
                CardDB.cardIDEnum id1 = card1.card.cardIDenum;
                CardDB.cardIDEnum id2 = card2.card.cardIDenum;
                bool isPair = 
                    (id1 == CardDB.cardIDEnum.GDB_118t && id2 == CardDB.cardIDEnum.GDB_118t2) || 
                    (id1 == CardDB.cardIDEnum.GDB_118t2 && id2 == CardDB.cardIDEnum.GDB_118t);
                if (isPair)
                {
                    this.allCharsOfASideGetDamage(false, 5);//AOE5

                    //从手牌移除这2张
                    this.owncards.Remove(card1);
                    this.owncards.Remove(card2);

                    //重新编号手牌位置
                    renumHandCards(this.owncards);

                    foundPair = true;
                    break;//一次只处理一堆，后续可能递归检查
                }
            }

            //如果找到配对，递归检查可能的新配对
            if (foundPair) CheckForGDB118Pair();
        }
        ...
        /// <param name="spawnKid">是否生成随从</param>
        /// <param name="oneMoreIsAllowed">是否允许额外的随从</param>
        /// <returns>召唤的随从，场面满则返回null<returns>
            //public Minion callKid(CardDB.Card c, int zonepos, bool own, bool spawnKid = true, bool oneMoreIsAllowed = false)
        public Minion callKidAndReturn(CardDB.Card c, int zonepos, bool own, bool spawnKid = true, bool oneMoreIsAllowed = false)
        {
            // 默认允许的最大随从数量为7，如果允许额外的随从，则加1
            int allowed = 7 + (oneMoreIsAllowed ? 1 : 0);
        ...
        public void discardCards(int num, bool own)
        {
            if (own)
            {
                ...
                int remainingDiscard = anz - numDiscardedCards;
                if (remainingDiscard > 0)
                {
                    handleRemainingCards(remainingDiscard, cardsValDict, discardedCardsBonusList);
                    numDiscardedCards += remainingDiscard;
                }
        ...
        private void handleCardDiscard(ref int bestCardPos, ref int bestCardValue, ref int bestSecondCardPos, ref int bestSecondCardValue, ref int numDiscardedCards, List<Handmanager.Handcard> discardedCardsBonusList)
        {
            if (bestCardPos == -1) return; // 没有可弃的牌时直接返回

            // 调整位置顺序（确保先弃后面的牌）
            if (bestSecondCardPos > bestCardPos && bestSecondCardPos != -1)
            {
                int tempPos = bestSecondCardPos;
                bestSecondCardPos = bestCardPos;
                bestCardPos = tempPos;
            ...
            int processed = Math.Min(numDiscardedCards, 2);
            for (int i = 0; i < processed; i++)
            {
                int cPos = (i == 0) ? bestCardPos : bestSecondCardPos;
                if (cPos == -1) continue;

                Handmanager.Handcard removedhc = this.owncards[cPos];
                this.owncards.RemoveAt(cPos);
                ...
                // 移除后更新后续卡牌的位置
                if (i == 0 && bestSecondCardPos > bestCardPos) bestSecondCardPos--;
            }
            numDiscardedCards = processed; // 更新实际处理数量
        }
        ...
        /// <param name="cardsValDict">卡牌价值字典。</param>
        /// <param name="discardedCardsBonusList">弃牌过程中获得奖励的卡牌列表。</param>
        private void handleRemainingCards(int remaining, Dictionary<int, int> cardsValDict, List<Handmanager.Handcard> discardedCardsBonusList)
        {
            if (remaining <= 0) return;

            // 按价值升序排列（弃掉低价值牌）
            var sortedCards = this.owncards.Take(remaining)
                .Select((hc, index) => new { hc, value = cardsValDict[hc.entity] })
                .OrderBy(x => x.value)
                .ToList();

            foreach (var cardInfo in sortedCards)
            {
                Handmanager.Handcard removedhc = cardInfo.hc;
                this.owncards.Remove(removedhc);
        ...
        private void handleMinionDiscardTriggers(int numDiscardedCards, List<Handmanager.Handcard> discardedCardsBonusList)
        {
            ...
            foreach (Minion m in this.ownMinions)
            {
                ...
                if (m.handcard != null)
                {
                    if (m.handcard.card != null)
                    {
                        if (m.handcard.card.sim_card != null)
                        {
                            // 执行效果触发
                            m.handcard.card.sim_card.onCardDicscard(this, m.handcard, m, numDiscardedCards);
                        }
            ...
            foreach (Handmanager.Handcard dc in discardedCardsBonusList)
            {
                if (dc == null) continue;  // 列表元素null检查
                if (dc.card != null)       // card对象null检查
                {
                    if (dc.card.sim_card != null) // sim_card对象null检查
                    {
                        dc.card.sim_card.onCardDicscard(this, dc, null, 0);
                    }
                }
        ...
        //    }
        //}
        public void useLocation(Minion own, Minion target, int penality)
        {
            // 获取当前要使用的地标卡牌
            CardDB.Card locationCard = own.handcard.card;
            this.evaluatePenality += penality;
