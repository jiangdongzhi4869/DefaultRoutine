        ...
        public List<Action> getMoveList(Playfield p, bool usePenalityManager, bool useCutingTargets, bool own)
        {
            ...
            foreach (Minion minion in usingMinions)
            {
                ...
                if (trgts.Count > 0)
                {
                    ...
                    {
                        //有目标指向的地标在这改
                        int useLocationPenalty = usePenalityManager ? pen.getUseLocationPenality(minion, trot, p) : -50;
                ...
                {
                    int useLocationPenalty = usePenalityManager ? pen.getUseLocationPenality(minion, null, p) : -50;
            ...
            //        foreach (var trot in trgts)
            //        {
            //            int useLocationPenalty = usePenalityManager ? pen.getUseLocationPenality(minion, trot, p) : -80;
            //            if (useLocationPenalty <= 499)
            //            {
            //                ret.Add(new Action(actionEnum.useLocation, null, minion, 0, trot, useLocationPenalty, -50));
            //            }
            //        }
            //    }
            //    else if (minion.handcard.card.sim_card.GetUseAbilityReqs().Length == 0)
